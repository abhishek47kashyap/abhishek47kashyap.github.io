...
First, let's define the model. Note how the `decoder` has the "inverse" of `Conv2d` as `ConvTranspose2d`. CNNs require special attention to the arguments `in_channels` and `out_channels`, the ones provided below will work out of the box with `28 X 28` MNIST images.
```
